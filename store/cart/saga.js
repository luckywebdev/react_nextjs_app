...
function* voucherDiscountSaga(payload) {
    try {
        yield put(voucherDiscountSuccess(payload));
    } catch (err) {
        console.error(err);
    }
}

function* addDeliverySaga(payload) {
    try {
        yield put(addDeliverySuccess(payload));
    } catch (err) {
        console.error(err);
    }
}

function* addUserInfoSaga(payload) {
    try {
        yield put(userInfoSaveSuccess(payload));
    } catch (err) {
        console.error(err);
    }
}
...
export default function* rootSaga() {
    ...
    yield all([takeEvery(actionTypes.VOUCHER_DISCOUNT, voucherDiscountSaga)]);
    yield all([takeEvery(actionTypes.ADD_DELIVERY, addDeliverySaga)]);
    yield all([takeEvery(actionTypes.USER_INFO_SAVE, addUserInfoSaga)]);
}
